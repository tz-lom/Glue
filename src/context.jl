...
Base.show(io::IO, ::MIME"text/plain", ::Type{ContextStack{Root}}) where {Root} =
    print(io, "[$Root]")

function Base.show(
    io::IO,
    type::MIME"text/plain",
    ::Type{ContextStack{Prev,Curr}},
) where {Prev,Curr}
    show(io, type, Prev)
    print(io, "[$Curr]")
end
...
Base.keys(::ContextPtr{Root,ContextStack{Parent,Child}}) where {Root,Parent,Child} =
    Base.keys(Child)
...
function Base.getindex(
    c::ContextPtr{Root,Prev},
    ::Type{T},
) where {Root,Prev,T<:AbstractContext}
    return ContextPtr{Root,ContextStack{Prev,T}}(c.root)
end

function Base.getindex(
    c::ContextPtr{Root,Stack},
) where {Root,Parent,Child,Stack<:ContextStack{Parent,Child}}
    return getfield(c.root, key(c.root, Stack))
end

function Base.getindex(
    c::ContextPtr{Root,Stack},
) where {Root,Child,Stack<:ContextStack{Child}}
...
function define_context(context_name, artifacts...)
    ...
    function enum_fields(context::Type{C}) where {C<:AbstractContext}
        return mapreduce(
            (x) -> enum_fields(x, ContextStack{C}),
            append!,
            keys(context),
            init = [],
        )
    ...
    function enum_fields(context::Type{C}, ::Type{Parent}) where {C<:AbstractContext,Parent}
        return mapreduce(
            (x) -> enum_fields(x, ContextStack{Parent,C}),
            append!,
            keys(context),
            init = [],
        )
...
function Base.show(
    io::IO,
    type::MIME"text/plain",
    ptr::ContextPtr{Root,ContextStack{Parent,A}},
) where {Root<:AbstractContext,Parent,A<:Artifact}
...
function Base.show(
    io::IO,
    type::MIME"text/plain",
    ptr::ContextPtr{Root,ContextStack{A}},
) where {Root<:AbstractContext,A<:Artifact}
    spaces = get(io, :spaces, "")
    show_artifact(io, ptr, A, spaces)
end
...
function Base.show(
    io::IO,
    type::MIME"text/plain",
    ptr::ContextPtr{Root,Stack},
) where {Root,Stack<:ContextStack}
    spaces = get(io, :spaces, "")
    if spaces == ""
